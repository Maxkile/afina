...
constexpr size_t DEFAULT_ANSWER_SIZE = 16;
...
void Connection::setStorage(std::shared_ptr<Afina::Storage> &pStorage) { this->pStorage = pStorage; }
        ...
        while ((new_read_bytes = read(_socket, command_buf + read_bytes, sizeof(command_buf)) - read_bytes) > 0) {
            ...
            while (read_bytes > 0) {
                ...
                if (command_to_execute && arg_remains == 0) {
                    ...
                    bool on_first_exec = answer.empty();
...
void Connection::DoWrite() {
    ...
    size_t answer_size, answers_fully_written = 0;
    int new_written_bytes = -1;

    answer_size = answer.size();

    struct iovec out[DEFAULT_ANSWER_SIZE];
    try {
        ...
        out[0].iov_len = out[0].iov_len + written_bytes;
        ...
        if ((new_written_bytes = writev(_socket, out, answer_size)) > 0) {
            ...
            for (auto answer_iter = answer.cbegin(); answer_iter != answer.cend();
                 ++answer_iter, ++answers_fully_written) {
                if ((answers_fully_written < answer_size) && (written_bytes - answer_iter->size() >= 0)) {
                    written_bytes -= answer_iter->size();
                } else {
                    break;
                }
            }

            answer.erase(answer.cbegin(), answer.cbegin() + answers_fully_written);
